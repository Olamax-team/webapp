...
import { useConfirmPasswordChangeModal, useVerifyPasswordChangeModal } from '../../lib/utils'

const ConfirmChangePasswordModal = () => {
  const { isOpen, onClose } = useConfirmPasswordChangeModal();
  const verifyPasswordChangeModal = useVerifyPasswordChangeModal();
  return (
    <Modal 
      isOpen={isOpen} 
      onClose={onClose}
    ...
    >
      <div className='flex flex-col gap-10'>
        ...
        <div className="flex items-center justify-between gap-4">
          <button className='w-full h-12 rounded-lg bg-primary text-white' onClick={() =>{onClose(); verifyPasswordChangeModal.onOpen();}}>
            Yes
          </button>
