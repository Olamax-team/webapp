import { useDonateConfirmationModal, useDonateCompletedModal } from '../../../lib/utils';
import Modal from '../../ui/modal'
import loading from "../../../assets/images/Loading 1.svg";
import { useEffect } from 'react';

const DonateConfirmationModal = () => {
    const { isOpen, onClose } = useDonateConfirmationModal();
    const openDonateCompleted = useDonateCompletedModal();

    useEffect(() => {
        if (isOpen) {
          const timer = setTimeout(() => {
            onClose(); 
            openDonateCompleted.onOpen(); 
          }, 2000); // 2 seconds delay
    
          return () => clearTimeout(timer);
        }
      }, [isOpen, onClose, openDonateCompleted]);
